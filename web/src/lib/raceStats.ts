import { calcWpm, creditedWordChars, isStationCorrect } from './wpm'
...
export type StationSample = {
  ...
  targetLen: number
  perfect: boolean
  /** Chars banked into race WPM (sum of fully correct words + their spaces). */
...
export function buildStationSample(args: {
  ...
  const creditedChars = creditedWordChars(args.target, args.typed)
  return {
    index: args.index,
    name: args.name,
